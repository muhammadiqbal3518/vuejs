  ...
  <div class="mdc-layout-grid">
    <div class="row mb-3">
      <div class="col-12 col-md-3 col-lg-1">
        ...
        <button type="button" class="btn btn-success form-control mb-3" @click="showModal()">Create</button>
      </div>
      <div class="col-12 col-md-9 col-lg-11">
        <input type="text" v-model="keyword" :onchange="searchItem()" class="form-control mb-3" placeholder="Search Here">
      </div>
    </div>
              ...
              <tbody>
                <tr v-for="(item, index) in filteredItems" :key="index">
                  ...
                  <td class="text-center">
                    <img v-if="item.image" height="100px" width="100px" :src=url+item.image class="img">
                    <img v-else height="100px" width="100px" src="@/assets/image/default_image-1024.webp" class="img">
                  </td>
                  <td>
                    <!-- <RouterLink :to="{ name: 'productUpdate', params: { productId: item.id } }" class="mdc-button mdc-button--raised icon-button filled-button--success">
                      <i class="material-icons mdc-button__icon">colorize</i>
                    </RouterLink> -->
                    <button class="btn btn-sm btn-primary" type="button" @click="editModal(item.id)">Edit</button>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="modal-body">
          <div class="row">
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary" :disabled=processing >{{  processing ? 'Saving ...' : 'Submit' }}</button>
            </div>
          </div>
        </form>
    </div>
          ...
          <div class="row">
            <div class="col-12">
                  ...
                  <div class="mb-3">
                      ...
                      <div class="mx-3 my-3">
                          <p>Current Image</p>
                          <img v-if="itemData.image" height="100px" width="100px" :src=url+itemData.image class="img">
...
export default {
    ...
    methods: {
        ...
        editModal(id) {
          ...
          axios.get('https://sub.mykavling.store/api/item/'+this.productId, {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          })
        ...
        updateProduct(id)
        {
            ...
            axios.post('https://sub.mykavling.store/api/item/'+id, formData, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            })
